...
- 년주, 월주, 일주, 시주 계산
- 태양 황경 기준 절기 시각으로 년주/월주 보정
- 지장간별 십성과 주요 신살 계산
- 모바일 상세 리포트 요약 카드, 스크롤 힌트, 현재 행 강조
- 출생시간 미입력 시 시주 제외
...
- 성별과 연간 음양으로 순행/역행을 정한다.
- 순행은 출생 이후 가장 가까운 절기, 역행은 출생 이전 가장 가까운 절기까지의 차이를 사용한다.
- `3일 = 1년`, `1일 = 4개월` 기준으로 월 단위 환산 후 `만 n년 n개월` 형태로 표시한다.
- 현재 앱은 정밀 절기 시각 API가 없으므로 기존 월주 계산과 같은 절기 날짜 기준을 사용한다.
